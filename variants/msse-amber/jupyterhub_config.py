...
#  Default: '/opt/mambaforge/envs/jupyterhub/share/jupyterhub'
# c.JupyterHub.data_files_path = '/opt/mambaforge/envs/jupyterhub/share/jupyterhub'
...
class PopulatingSpawner(SystemdSpawner):
    async def start(self):
        ret = await SystemdSpawner.start(self)

        username = self.user.name
        userdir = f"/var/lib/{username}"
        assert os.path.exists(userdir)

        Repo.clone_from("https://github.com/chem-274-A-master/molecular-dynamics-lab2.git", f"{userdir}/git")
        shutil.rmtree(f"{userdir}/git/.git")
        distutils.dir_util.copy_tree(f"{userdir}/git", userdir)
        shutil.rmtree(f"{userdir}/git")

        return ret

c.JupyterHub.spawner_class = PopulatingSpawner
